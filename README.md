...
> Python 3.11+ · Node.js/npm React dashboard · SQLite service store · Windows local metadata 수집 지원 · dashboard/report/OpenAPI 자동 생성
...
-> SQLite service store / local worker boundary
...
flowchart LR
  ...
  AI --> O
  O --> DB["outputs/service/layertrace.sqlite3"]
...
React dashboard는 같은 최신 결과를 `web/public/latest-result.json`에서 읽습니다.
...
│   ├── service_store.py    # SQLite run/incident/task store
│   ├── service_worker.py   # local worker boundary
│   ├── service_api.py      # stdlib REST API
